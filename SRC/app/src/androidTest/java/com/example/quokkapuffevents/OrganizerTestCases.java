...
public class OrganizerTestCases {
    ...
    public User accessOrganizerDashboard() {
        User mockOrganizer = createTestOrganizer();
        try (ActivityScenario<LoginActivity> scenario =
                     ActivityScenario.launch(LoginActivity.class)) {

            onView(withId(R.id.login_email_address)).perform(typeText(mockOrganizer.getEmail()));
            closeSoftKeyboard();
...
//                    .perform(click());
    ...
    public void SendNotificationToChosenEntrants() {
        User organizer = accessOrganizerDashboard();
        ...
        try {
            ...
            assertEquals("Should have 2 accepted users", 2, acceptedCount);

            assertTrue("cancelled1 should be in cancelled list", acceptedUserIds.contains(accepted1.getId()));
            assertTrue("cancelled2 should be in cancelled list", acceptedUserIds.contains(accepted2.getId()));
            ...
            db.UsersInEvent(event, users -> {
                ...
                assertEquals("Should have 2 accepted users in filtered list", 2, acceptedUsers.size());

                boolean hasCancelled1 = acceptedUsers.stream().anyMatch(u -> u.getId().equals(accepted1.getId()));
                boolean hasCancelled2 = acceptedUsers.stream().anyMatch(u -> u.getId().equals(accepted2.getId()));
                assertTrue("accepted1 should be in accepted users list", hasCancelled1);
                assertTrue("accepted2 should be in accepted users list", hasCancelled2);
            });
    ...
    /**
     * US 02.05.01 As an organizer I want to send a notification to cancelled entrants.
     */
    @Test
    public void SendNotificationToCancelledEntrants() {
        ...
        User cancelled2 = db.CreateUser("cancelled2@test.com", 0, "pass",
                "accepted2", "Jane", "Smith", "5553333333"
        );
        ...
        try {
            ...
            assertEquals("Should have 2 accepted users", 2, cancelledCount);
